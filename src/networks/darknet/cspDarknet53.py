...
class CSPDarkNet53(nn.Module):
    def __init__(self, num_classes=3, used_blocks_num=3, divider=4):
        super(CSPDarkNet53, self).__init__()
        self.module_list = nn.ModuleList()

        # set parameters
        in_out_channels = np.array([
                    [32, 64],
                    [64, 128],
                    [128, 256],
                    [256, 512],
        ...
        transition_in_out_channels = np.array([
                    [64, 64],
                    [128, 64],
                    [256, 128],
                    [512, 256],
        ...
        concat_in_out_channels = np.array([
                    [128, 64],
                    [128, 128],
                    [256, 256],
                    [512, 512],
        ...
        # first conv
        conv1 = ConvBnMish(input_channels = 3,output_channels = in_out_channels[0][0], kernel_size = 3, stride = 1)
        self.module_list.append(conv1)
        
        # downsample and residuals
        for idx in range(0, used_blocks_num):
        ...
        self.fc_channels = in_out_channels[used_blocks_num-1][1]
    ...
    def forward(self, x):
        ...
        x = self.avg_pool(x)
        x = x.view(-1, self.fc_channels)
        x = self.fc(x)

        return x

if __name__ == "__main__":
    body = CSPDarkNet53(divider=8)
    print(body)
